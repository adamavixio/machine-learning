...
const DQNAgent = ml.rl.DQNAgent(Cube2x2State);
...
pub fn main() !void {
    ...
    for (0..pretrain_iters) |iter| {
        ...
        if (iter % 100 == 0 or iter == pretrain_iters - 1) {
            ...
            for (0..6) |i| {
                const action = try agent.selectAction(&depth1_states[i], &tensor_ctx, &ad_ctx, rng);
    ...
    const evaluatePolicy = struct {
        fn eval(
            ...
            _: *GradContext, // Unused, only needed for forward pass
            ad_ctx_ptr: *AutodiffContext,
            rng_val: std.Random,
        ) !EvalResult {
            ...
            for (0..num_eval) |_| {
                ...
                for (0..50) |_| { // Max 50 moves
                    ...
                    const action_eval = try agent_ptr.selectAction(&state_onehot_eval, tensor_ctx_ptr, ad_ctx_ptr, rng_val);
    ...
    for (0..total_episodes) |episode| {
        ...
        for (0..config.max_episode_steps) |step_idx| {
            ...
            // Store experience with episode ID for age tracking
